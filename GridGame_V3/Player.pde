...
class Player {
  ...
  void move(InputHandler input, BoardManager board, PlayerManager playerManager) {
    int dx = 0, dy = 0;
    if (id == 0) { //WASD keys
    ...
    //if player completed objective, gain points (pts value determined by GPT-2)
    if (board.board[nx][ny].milestone && !board.board[nx][ny].visited) {
      board.board[nx][ny].visited = true;
      playerManager.score += playerManager.gainThreshold;
  }
    //if player attempts to traverse off-limits tile (hole), lose points (pts value again determined by GPT-2)
    if (board.board[nx][ny].hole) {
      playerManager.score -= playerManager.lossThreshold;
    }
  }
